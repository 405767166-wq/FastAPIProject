...
| `memory.py` | `MemoryStore(MeetingStore)`：V1 默认实现。内部用两个 dict（`_meetings`、`_words`）+ `itertools.count` 自增 seq（模拟自增主键）+ `threading.Lock`（防并发写）；时间戳用 UTC ISO 格式（`_now_iso`）；列表按 id 倒序分页，支持 status/keyword 筛选。 |
